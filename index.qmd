...
O site da [série histórica com a inflação dos EUA](https://data.bls.gov/pdq/SurveyOutputServlet) pode ser acessado acessando o link.
...
4.  Fizemos um join entre a `imdb` e a `cpi_usa`;
...
Continuamos usando somente os filmes cujas as colunas `orcamento` e `receita` estavam em dólares (\$) e, em seguida, apresentamos em formato de tabela os filmes com as maiores notas médias *ponderadas*.
...
Para representar visualmente a distribuição da nota atribuída a `r filme_fav` por idade, nos inspiramos em um gráfico feito por Cedric Scherer. O link com o gráfico feito por ele está disponel [neste link](<https://www.cedricscherer.com/2023/10/26/yet-another-how-to-on-labelling-bar-graphs-in-ggplot2/>).
